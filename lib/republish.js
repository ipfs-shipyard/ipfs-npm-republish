...
var republish = {
  processDependency: function(key, value, registry, tmpDir, localIPFSgateway, cb) {
    ...
    republish.loadPackument(key, registry, function(packument) {
      var tarballPath = tmpDir+'/'+key+'-'+value.version+'.tgz'
      ...
      republish.addTarballIPFS(key, value, tarballPath, packument, localIPFSgateway, tmpDir, cb)
    });
  },

  addTarballIPFS: function(key, value, tarballPath, packument, localIPFSgateway, tmpDir, cb) {

    // download the tarball to ROOT
    republish.downloadTarball(value.resolved, tarballPath, function() {
      // ipfs add tarball
      exec('ipfs add --cid-base=base32 --quiet '+tarballPath, (err, stdout, stderr) => {
        ...
        var packumentVersion = packument.versions[value.version]
        packumentVersion.dist.tarball = localIPFSgateway + 'ipfs/' + tarballCid

        var versions = {}
        versions[value.version] = packumentVersion
