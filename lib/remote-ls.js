...
RemoteLS.prototype._loadPackageJson = function (task, done) {
  ...
  var couchPackageName = name.replace('/', '%2F')
  ...
  var url = this.registry.replace(/\/$/, '') + '/' + couchPackageName

  if (_this.packuments[url] != undefined) {
    var obj = _this.packuments[url]
    try {
    ...
    var request = protocol.get(url, {headers: {accept: 'application/vnd.npm.install-v1+json'}}, function(res) {
      ...
      res.on("end", () => {
        obj = JSON.parse(body);
        _this.packuments[couchPackageName] = obj
        try {
          if (_this.verbose) console.log('loading:', name + '@' + task.version)
          _this._walkDependencies(task, obj, done)
        } catch (e) {
